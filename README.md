...
> 🏃 **달리고 나서 글이 올라가기까지의 순서는 [`WORKFLOW.md`](WORKFLOW.md) 한 장에 있습니다.**
> 이 문서는 스킨 설치와 기능 설명입니다.
...
| 📑 **자동 목차 · 읽기 진행률** | 제목이 3개 이상이면 목차가 생깁니다. 넓은 화면에서는 글 오른쪽에 따라다니며 지금 읽는 곳을 표시하고, 상단에 진행률 바가 표시됩니다. |
...
### 2. 지도 데이터를 어디서 읽을지 정하기 (한 번만)

`images/mapData.js` 의 `coursesUrl` 이 이걸 정합니다.

| | 지도를 갱신하려면 | 반영까지 |
| --- | --- | --- |
| **주소를 채워둔다** (기본) | `git push` | 5분쯤 |
| 비워둔다 | `courses.json` 을 스킨에 다시 업로드 | 바로 |

```js
coursesUrl: 'https://raw.githubusercontent.com/<계정>/<저장소>/main/images/courses.json',
```

- 저장소가 **공개**여야 합니다.
- 주소를 못 읽으면 스킨에 올려둔 `courses.json` 으로 **알아서 되돌아갑니다.** 지도가 빈 화면이 되지 않습니다.
- `mapData.js` 자체는 스킨 파일이라, 이 주소를 바꿀 때만 다시 올리면 됩니다.
- `map.js` 나 `mapData.js` 를 다시 올렸으면 `skin.html` 의 `?v=9` 숫자를 올려 캐시를 비우세요.
...
**스킨 편집은 열지 않습니다. 푸시하면 지도가 갱신됩니다.**

```bash
# 1. gpx/ 폴더에 활동 파일(.tcx / .gpx)을 넣고 빌드
node tools/build-courses.mjs

# 2. 글을 티스토리에 발행하고 글 번호를 확인합니다 (예: 213)

# 3. 그 번호를 코스에 연결하면서 코스 이름도 정합니다
node tools/link-course.mjs run-2026-09-21-0828 213 --title "여의도 한강 4K"

# 4. 푸시합니다. 5분쯤 뒤 지도에 반영됩니다
git add images/courses.json && git commit -m "🗺️ 여의도 한강 4K" && git push
```

> 글을 먼저 발행하는 순서인 이유는, 지도 데이터에 글 주소가 들어가기 때문입니다.
> 나중에 연결하면 커밋을 한 번 더 하게 됩니다.

- `gpx/out/<아이디>.md` 에 글에 붙여넣을 표가 생깁니다.
- 코스 이름을 정하지 않으면 `2026-09-21 경기 4K` 처럼 날짜·지역·거리로 자동으로 붙습니다.
  (RunGap 파일명이 그대로 지도에 뜨지 않게 하려는 것입니다)
- `--title` 로 정한 이름과 연결한 글 주소는 **다시 빌드해도 유지됩니다.**
- `gpx/sample.gpx` 같은 예시 파일은 빌드에서 빠집니다. 예시까지 보려면 `--with-sample`.
- 활동 원본(`gpx/*.tcx`)은 `.gitignore` 에 있습니다. 공개 저장소에 올라가지 않습니다.
...
- **명령어** — 코스 이름 일부만 알면 됩니다. 이름도 같이 정할 수 있습니다.

  ```bash
  node tools/link-course.mjs                                  # 코스 목록 · 연결 상태
  node tools/link-course.mjs 여의도 213 --title "여의도 한강 4K"
  ```

  `images/courses.json` 과 `gpx/meta.json` 두 곳에 함께 적습니다. 다시 빌드할 필요 없이
  바로 업로드하면 되고, 나중에 다시 빌드해도 이름과 주소는 그대로 남습니다.
...
| `- [x]` / `- [ ]` | 체크리스트 |
| `##` 제목 3개 이상 | **자동 목차** (넓은 화면은 오른쪽에 고정 · 좁은 화면은 글 맨 위) + 상단 읽기 진행률 |
...
글을 발행한 뒤 `node tools/link-course.mjs <코스> <글번호>` 로 넣어 주세요.
...
**활동 파일을 고쳤는데 지도가 그대로예요**
`coursesUrl` 을 쓰고 있다면 **푸시했는지** 확인하고 5분쯤 기다리세요. GitHub 이 그만큼 캐시합니다.
비워두고 스킨 파일을 쓰고 있다면 `courses.json` 을 다시 업로드하세요.
`map.js` · `mapData.js` 자체를 고쳤을 때는 `skin.html` 의 `?v=9` 숫자를 올려야 반영됩니다.

**지도 데이터를 CDN 에서 읽는지 어떻게 확인하나요**
브라우저 개발자도구 **Network** 탭에서 `courses.json` 요청 주소를 보세요.
`raw.githubusercontent.com` 이면 CDN, 블로그 주소면 스킨 파일입니다.
CDN 을 못 읽어 되돌아간 경우에는 **Console** 에 `[러닝맵]` 으로 시작하는 경고가 찍힙니다.
